...
def run_diagnostics():
    ...
    for cond in conditions:
        for adapt_active in modes:
            ...
            T8_variance_stats = {
                "correct": {"count": 0, "sum": 0.0, "sumsq": 0.0, "hist": [0]*20},
                "wrong":   {"count": 0, "sum": 0.0, "sumsq": 0.0, "hist": [0]*20},
            }
            ...
            for step, batch in enumerate(tqdm(data_loader, desc=f"Diagnostics [{cond} | {mode_str}]")):
                ...
                with torch.no_grad():
                    ...
                    # Add Test 8: Cross-View Variance Discrimination
                    x_yaw = torch.roll(proj_in, shifts=14, dims=3)
                    enc_yaw, _, _ = model.encode(x_yaw)
                    raw_yaw = F.normalize(enc_yaw[valid_mask_3d])
                    del x_yaw, enc_yaw
                    
                    x_drop = F.dropout2d(proj_in, p=0.15, training=True)  # force-on regardless of model.eval()
                    enc_drop, _, _ = model.encode(x_drop)
                    raw_drop = F.normalize(enc_drop[valid_mask_3d])
                    del x_drop, enc_drop
                    ...
                    stacked = torch.stack([sims, sims_yaw, sims_drop], dim=0)
                    view_variance = stacked.var(dim=0)
                    
                    high_conf_mask = sims > 0.75
                    group_correct = high_conf_mask & correct_mask
                    group_wrong = high_conf_mask & ~correct_mask
                    
                    for group_name, group_mask in [("correct", group_correct), ("wrong", group_wrong)]:
                        vals = view_variance[group_mask]
                        if vals.numel() == 0:
                            continue
                        g = T8_variance_stats[group_name]
                        g["count"] += vals.numel()
                        g["sum"] += vals.sum().item()
                        g["sumsq"] += (vals ** 2).sum().item()
                        # variance of cosine-sim-like values is small; scale bins to [0, 0.1] range
                        bin_idx = (vals / 0.1 * 20).clamp(0, 19).long()
                        for b in bin_idx.tolist():
                            g["hist"][b] += 1
