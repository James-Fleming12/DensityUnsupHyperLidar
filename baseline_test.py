...
from dataset.kitti.parser import SemanticKitti
from modules.HDC_utils import DensityModel
from unsup_kitti import LiDARCorruptionWrapper
...
def run_diagnostics():
    ...
    # Setup Dataset
    baseline_parser = SemanticKitti(
        root=DATA_DIR,
        vocab=None,
        **ARCH["dataset"]
    )
    
    print("Setting up SemanticKITTI-C loaders...")
    train_dataset = baseline_parser.get_train_set().dataset
    
    # Test on Snow Severity 3
    snow_train = LiDARCorruptionWrapper(train_dataset, corruption_type="snow", severity=3)
    snow_loader = DataLoader(snow_train, batch_size=ARCH["train"]["batch_size"], shuffle=False, num_workers=ARCH["train"]["workers"], drop_last=False)
    
    print("Loading Original DensityModel Baseline...")
    # Use exact same model initialization as unsup_kitti-c.py
    ...
    print("\n" + "="*60)
    print("Running Diagnostic Adaptation on SNOW (Severity 3) using Standard Baseline")
    ...
    T6_drift = {c: {"cos_to_origin": []} for c in range(NUM_CLASSES)}  # Excluded delta_dirs from JSON to save massive space
    ...
    for step, batch in enumerate(tqdm(snow_loader, desc="Diagnostics [SNOW Baseline]")):
        ...
        with torch.no_grad():
            # 1. Base Extraction (Same as Baseline)
            enc_base, _, _ = model.encode(proj_in)
            valid_mask_3d = (enc_base.abs().sum(dim=1) > 0)
            
            # Local density (2D avg pool on valid mask)
            ...
            # Decision using standard baseline feature space
            ...
            # Test 5: Subcluster tracking
            if step % 50 == 0:
                for c_id in range(NUM_CLASSES):
                    c_mask = (active_labels == c_id)
                    if c_mask.sum() > 0:
                        c_encs = raw_base[c_mask]
                        _, sub_idx = model.get_max_subcluster_similarity(c_encs, c_id, distance_sensitivity=1.0)
        ...
        # Apply the exact baseline unsupervised update from unsup_kitti-c.py
    ...
    out_path = os.path.join(SAVE_DIR, "baseline_diagnostics_snow.json")
    with open(out_path, "w") as f:
        json.dump(stats, f, indent=4)
        
    print(f"\nDiagnostics completed! JSON Data saved to {out_path}")
